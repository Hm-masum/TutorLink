...
const UpdateStudentPostForm = ({ postData }: { postData: IStudentPost }) => {
  const form = useForm({
    defaultValues: {
      ...
      studentGender: postData.studentGender,
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    try {
      ...
      const res = await updateStudentPost(updateData, postData._id);
      if (res?.success) {
  ...
  return (
    <div className="border-2 border-gray-300 rounded-xl flex-grow w-full p-5">
      <SectionTitle
        ...
        colorWord="Post"
        subtitle="Update Your Post to Find the Best Tutor Match"
      />
...
export default UpdateStudentPostForm;
