...
const TutorPostCard = ({ posts }: { posts: ITutorPost }) => {
  return (
    <div className="p-4 md:p-8 bg-white border dark:bg-black shadow-sm hover:border-purple-800 rounded-lg overflow-hidden space-y-2 w-full">
        ...
        <div className="md:flex items-center gap-4 w-[50%]">
          <Avatar className="h-10 w-10">
            <AvatarImage
              ...
              src={posts?.tutorId?.image || "https://github.com/shadcn.png"}
            />
            <AvatarFallback>Tutor</AvatarFallback>
          </Avatar>
          <p>{posts.tutorId.email}</p>
        </div>
        <Link href={`/tutor-posts/${posts._id}`}>
