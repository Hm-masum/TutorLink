...
import { DeleteStudentPost } from "@/services/StudentPost";
...
const ManageStudentPosts = ({ posts }: { posts: IStudentPost[] }) => {
  const router = useRouter();
  const [isModalOpen, setModalOpen] = useState(false);
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [selectedItem, setSelectedItem] = useState<string | null>(null);

  const handleDelete = (data: IStudentPost) => {
  ...
  const handleDeleteConfirm = async () => {
    try {
      if (selectedId) {
        const res = await DeleteStudentPost(selectedId);
        if (res.success) {
  ...
  const columns: ColumnDef<IStudentPost>[] = [
    {
      accessorKey: "title",
      header: "Title",
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Button
          ...
          onClick={() => router.push(`/student-posts/${row.original._id}`)}
        >
          Details
        </Button>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <Button
          className="bg-purple-700"
          onClick={() =>
            router.push(`/student/update-post/${row.original._id}`)
          }
        >
...
export default ManageStudentPosts;
