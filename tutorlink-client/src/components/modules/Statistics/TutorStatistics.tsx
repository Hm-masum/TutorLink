...
const TutorStatistics = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      const { data: applicants } = await applicantInfoOfTutorPost();
      const { data: applications } = await myApplicationInfoOfStudentPost();
    ...
    <div className="min-h-[100vh]">
      <div className="shadow text-center font-semibold rounded-lg flex flex-col md:flex-row w-full">
        ...
        <div className="space-y-3 text-blue-700 py-7 px-5 w-full md:w-1/3 border">
          ...
          <div>Number of applicant in my post {applicants.length}</div>
        </div>
        ...
        <div className="space-y-3 py-7 px-5 w-full md:w-1/3 border">
          ...
          <div>
            Number of my application in student post {applications.length}
          </div>
        </div>
      </div>
